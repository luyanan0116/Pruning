...
# 导入 LCB 计算步骤
from .lcb_utils import compute_lcb_weight_metric
...
def prune_lcb(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            layer_activations = []
            for act in real_activations[name]:
                if len(act.shape) == 2:
                    act = act.unsqueeze(0) 
                layer_activations.append(act)
                
            W_metric = compute_lcb_weight_metric(W, layer_activations, scaler_row, args)
